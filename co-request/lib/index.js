...
var debug = require('debug')('coRequest');
...
function share(defaults) {
  ...
  defaults.gunzip = defaults.gunzip !== false;
  defaults.netrc = Netrc(defaults.netrc);

  return coRequest;

  function* coRequest(uri, options) {
    ...
    headers['user-agent'] = headers['user-agent'] || 'https://github.com/shaozj/learnNodejs/tree/master/co-request';
    ...
    while (true) {
      ...
      // retires is on a per-URL-request basis
      ...
      if (options.string || options.json)
        res.text = res.buffer ? res.buffer.toString('utf8')
